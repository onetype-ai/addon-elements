...
Field reference: `id` (dashed, becomes the tag name), `addon`, `icon`, `name`, `description`, `category`, `author`, `collection` (presentation and grouping), `config` (property defines), `metadata`, `example` (property sets for previews), `render`.
...
`Render(name, data, attributes, slots)` returns the live render instance: `render.Element` is the DOM node, state changes patch it in place. Registering is the same call with a function: `elements.Render(name, callback)`.
